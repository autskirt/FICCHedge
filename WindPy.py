...
class w:
    ...
    def wdata2dfdt(out, apiout, isTDays = False):
        ...
        if idx:
            ...
            pdver = int(pdver.replace(".", ""))
            if pdver >= 222:
                dfdt = df.map(w.transformNulldate2NaT)
            else:
                dfdt = df.applymap(w.transformNulldate2NaT)
            dft = dfdt.T
            dft.columns = idx
...
##                return
    ...
    def start(waitTime=120, showmenu=True):
            ...
            outdata=w.WindData();
            if(w.isconnected()):
                outdata.setErrMsg(0,"Already connected!");
                return outdata;
            ...
            if(err==0):
                ...
                print("");
                print("COPYRIGHT (C) 2024 WIND INFORMATION CO., LTD. ALL RIGHTS RESERVED.");
